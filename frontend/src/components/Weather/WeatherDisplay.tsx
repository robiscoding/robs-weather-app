...
function WeatherConditionIcon({ code }: { code: number }) {
  ...
  if (code >= 200 && code < 300) return <CloudLightning className={className} strokeWidth={strokeWidth} />;
  if (code >= 300 && code < 400) return <CloudDrizzle className={className} strokeWidth={strokeWidth} />;
  if (code >= 500 && code < 600) return <CloudRain className={className} strokeWidth={strokeWidth} />;
  if (code >= 600 && code < 700) return <CloudSnow className={className} strokeWidth={strokeWidth} />;
  if (code >= 700 && code < 800) return <CloudFog className={className} strokeWidth={strokeWidth} />;
  if (code === 800) return <Sun className={className} strokeWidth={strokeWidth} />;
  if (code === 801) return <CloudSun className={className} strokeWidth={strokeWidth} />;
...
function unitLabels(units: string) {
  switch (units) {
    case "imperial": return { temp: "°", speed: "mph", dist: "mi" };
    case "metric":   return { temp: "°", speed: "m/s", dist: "km" };
    default:         return { temp: "K", speed: "m/s", dist: "m" };
  }
}
    ...
    <div className="w-full max-w-sm mx-auto flex flex-col items-center gap-8">
      <div className="flex items-center gap-1.5 text-white/70">
        ...
        <span className="text-sm font-medium tracking-wide">
          {locationName ?? `${weather.location.lat.toFixed(2)}, ${weather.location.lon.toFixed(2)}`}
        </span>
      </div>
      ...
      <div className="flex flex-col items-center gap-1">
        ...
        <p className="text-7xl sm:text-[7rem] leading-none font-extralight text-white tracking-tighter">
          {Math.round(weather.temp)}{u.temp}
        </p>
        <p className="text-lg font-medium text-white/90">{weather.condition.label}</p>
        <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1 mt-1 text-sm text-white/60">
          <span>Feels like {Math.round(weather.feels_like)}{u.temp}</span>
          <span className="text-white/30">·</span>
          <span className="inline-flex items-center gap-0.5">
            <ArrowUp className="h-3 w-3" />
            {Math.round(weather.temp_max)}{u.temp}
          </span>
          <span className="inline-flex items-center gap-0.5">
            <ArrowDown className="h-3 w-3" />
            {Math.round(weather.temp_min)}{u.temp}
          </span>
        </div>
      </div>
